...
def test_get_valid_users_info_as_dict():
    ...
    with patch("builtins.open", mock_open(read_data=mock_csv_data)):
        ...
        assert result == [
            ...
            {"first_name": "Jane", "last_name": "Doe", "email": "jane.doe@example.com", "password": "password456"}
        ]
...
def test_get_invalid_users_from_file():
    mock_csv_data = "email,password\ninvalid.email@example.com,wrongpassword\nanother.invalid@example.com,12345\n"
    with patch("builtins.open", mock_open(read_data=mock_csv_data)):
        result = get_invalid_users_from_file()
        assert result == [
            {"email": "invalid.email@example.com", "password": "wrongpassword"},
            {"email": "another.invalid@example.com", "password": "12345"}
        ]
